...
public final class RenderHandler extends Handler {
    ...
    private int mTexId = -1;    //已经绑定的将要显示的纹理ID ;
    ...
    public static final RenderHandler createHandler(final String name) {
        if (DEBUG)
            Log.v(TAG, "createHandler:name=" + name);
        final RenderThread thread = new RenderThread(name);
        thread.start();
        return thread.getHandler();
    }

    //CameraClient对象的addSurface()最终final调用的addSurface()里surface传入这里处理 :
    public final void setEglContext(final EGLContext shared_context, final int tex_id, final Object surface, final boolean isRecordable) {
        if (DEBUG)
            Log.i(TAG, "RenderHandler:setEglContext:");
    ...
    private static final class RenderThread extends Thread {
        ...
        public final void handleSetEglContext(final EGLContext shard_context, final Object surface, final boolean isRecordable) {
            if (DEBUG)
                Log.i(TAG_THREAD, "setEglContext:");
            release();
            synchronized (mSync) {
        ...
        public final void run() {
            ...
            Looper.loop();
            if (DEBUG)
                Log.v(TAG_THREAD, "finishing");
            release();
            synchronized (mSync) {
                mHandler = null;
            }
            if (DEBUG)
                Log.v(TAG_THREAD, "finished");
        }

        private final void release() {
            if (DEBUG)
                Log.v(TAG_THREAD, "release:");
            if (mDrawer != null) {
                mDrawer.release();
                mDrawer = null;
            }
        ...
        private final void clear() {
            if (DEBUG)
                Log.v(TAG_THREAD, "clear:");
            mTargetSurface.makeCurrent();
            //设置背景帧颜色
            GLES20.glClearColor(0, 0, 0, 1);        //清除颜色帧缓冲区的数据 ;
            //重绘的背景颜色
