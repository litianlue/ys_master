...
/** Converts an unaligned two-byte little-endian integer into an int16 //将未分配的两字节小端整数转为为int16类型数据 */
...
#ifdef UVC_DEBUGGING
#include <libgen.h>
#ifdef __ANDROID__	// add for android Mr_Wen@sereneginat
...
/** Video interface subclass code (A.2) */  //视频接口的子类代码（A.2）
...
struct uvc_stream_handle {
  ...
  uint8_t *outbuf, *holdbuf;        //图像数据的帧缓冲区（不必要的额外缓冲区？）
  pthread_mutex_t cb_mutex;
  pthread_cond_t cb_cond;
  pthread_t cb_thread;
  uint32_t last_polled_seq;
  uvc_frame_callback_t *user_cb;    //用户的视频数据帧回调函数 :
  void *user_ptr;
  struct libusb_transfer *transfers[LIBUVC_NUM_TRANSFER_BUFS];      //从libusb传递过来的包含usb数据的结构体 数组
  uint8_t *transfer_bufs[LIBUVC_NUM_TRANSFER_BUFS];
  struct uvc_frame frame;       //视频数据帧 :
...
struct uvc_device_handle {
  ...
  struct libusb_transfer *status_xfer;      //从libusb传递过来的包含usb数据的结构体
  ...
  摄像机是否iSight，将每帧的一个标题 */
  uint8_t is_isight;
  uint8_t reset_on_release_if;	// XXX whether interface alt setting needs to reset to 0.   //XXX接口是否ALT设置需要重置为0
};
