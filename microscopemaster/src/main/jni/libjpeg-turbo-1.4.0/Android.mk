...
#リンクするライブラリを指定(静的モジュールにする時は不要)  #指定链接的图书馆（静模块里的时候不需要）。
#LOCAL_LDLIBS := -L$(SYSROOT)/usr/lib -ldl	# to avoid NDK issue(no need for static library) #为了避免NDK的问题（不需要静态库）

#このモジュールを外部モジュールとしてリンクする時のライブラリを指定 #把这个模块作为外部模块连接的时候的图书馆

LOCAL_ARM_MODE := arm

# コンパイル・リンクするソースファイル #编译程序的源文件
LOCAL_SRC_FILES := \
