...
// XXX assertはNDEBUGが定義されていたら引数を含めて丸ごと削除されてしまうので        //assert XXX是NDEBUG被定义了参数在内的整个被删除了
// 関数実行を直接assertの引数にするとその関数はNDEBUGの時に実行されなくなるので注意   //函数实行直接assert的参数的话那个函数是NDEBUG的时候要注意运行
#include <assert.h>
