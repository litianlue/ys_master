...
private:
	...
	ObjectArray<uvc_frame_t *> previewFrames;   //保存UVC_Frame_t图像帧数据的泛型数组 :
	int previewFormat;
	size_t previewBytes;
//
...
// improve performance by reducing memory allocation
	...
	pthread_mutex_t pool_mutex_one;
...
//
	...
	int g_map[256];
	int b_map[256];
	void changeColor(uint8_t REF_RED,uint8_t REF_GREEN,uint8_t REF_BlUE);
	void recolor(uint8_t * buf, int width,int height,int bpp);
	void Add_edge(uint8_t * buf, int width,int height, int bpp);
public:
