 ...
 *
 * File name: yeespec_usb_UVCCamera.cpp
 *
...
//nativeCreate() : Java层创建UVCCamera()对象调用的底层 , 返回当前连接的相机的ID : id_camera
...
/*static jdouble nativeGetFreShness(JNIEnv *env, jobject thiz,
								  ID_TYPE id_camera,jint heigh,jint width,jint bpp,jstring buffs) {

	jdouble result = JNI_ERR;
	ENTER();
	unsigned char * mchar;



	const char *c_usbfs = env->GetStringUTFChars(buffs, JNI_FALSE);
	UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
	if (LIKELY(camera)) {
		//result = camera->getFreShness(mchar,heigh,width,bpp);
	}
...
//Java层中UVCCamera对象startPreview()调用的底层 : 开始预览图像 :
...
//Java层中UVCCamera对象stopPreview()调用的底层 : 停止预览图像 :
...
//2016.07.19 : IFrameCallback 最终调用到这一步 :
...
static jint nativeSetColor(JNIEnv *env, jobject thiz,
							  ID_TYPE id_camera, jint color) {

	jint result = JNI_ERR;
	ENTER();
	UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
	if (LIKELY(camera)) {
		result = camera->setColor(color);
	}
	RETURN(result, jint);
}
static jint nativeSetGray(JNIEnv *env, jobject thiz,
						   ID_TYPE id_camera, jint gray) {

	jint result = JNI_ERR;
	ENTER();
	UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
	if (LIKELY(camera)) {
		result = camera->setColor(gray);
	}
	RETURN(result, jint);
}
...
//======================================================================
// Java mnethod correspond to this function should not be a static mathod       //java方法对应于这种功能不应该是一个静态的方法
...
static JNINativeMethod methods[] = {
	...
	{ "nativeSetStatusCallback",		"(JLcom/yeespec/libuvccamera/usb/IStatusCallback;)I", (void *) nativeSetStatusCallback },
	{ "nativeSetButtonCallback",		"(JLcom/yeespec/libuvccamera/usb/IButtonCallback;)I", (void *) nativeSetButtonCallback },
	...
	{ "nativeSetFrameCallback",			"(JLcom/yeespec/libuvccamera/usb/IFrameCallback;I)I", (void *) nativeSetFrameCallback },

	//{ "nativeGetFreShness",				"(JIILjava/lang/String;)I", (void *) nativeGetFreShness},
...
//	{ "nativeGetFocusSimple",			"(J)I", (void *) nativeGetFocusSimple },
	...
	{ "nativeSetColor",                 "(JI)I", (void*) nativeSetColor},

	{ "nativeSetGray",                 "(JI)I", (void*) nativeSetGray},
...
int register_uvccamera(JNIEnv *env) {
	...
	if (registerNativeMethods(env,
		"com/yeespec/libuvccamera/usb/UVCCamera",
