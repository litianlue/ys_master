...
UVCCamera::~UVCCamera() {
    ...
    release();
    if (mContext) {
        uvc_exit(mContext);     //关闭UVC上下文，关闭任何积极活动的相机。
        mContext = NULL;
    }
    if (mUsbFs) {
        free(mUsbFs);
...
int UVCCamera::connect(int vid, int pid, int fd, const char *usbfs) {
    ...
    if (!mDeviceHandle && fd) {
        ...
        if (UNLIKELY(!mContext)) {
            result = uvc_init2(&mContext, NULL, mUsbFs);    //初始化UVC上下文。
        ...
        clearCameraParams();
        fd = dup(fd);
        result = uvc_find_device2(mContext, &mDevice, vid, pid, NULL,
                                  fd);      //找出由供应商，产品和/或序列号确定的相机。
        if (LIKELY(!result)) {
            result = uvc_open(mDevice, &mDeviceHandle);         //打开紫外线装置。
            if (LIKELY(!result)) {
#if LOCAL_DEBUG
...
//======================================================================
// カメラのサポートしているコントロール機能を取得する    //取得照相机的支持的控制功能
int UVCCamera::getCtrlSupports(uint64_t *supports) {
    ENTER();
    uvc_error_t ret = UVC_ERROR_NOT_FOUND;
    if (LIKELY(mDeviceHandle)) {
        if (!mCtrlSupports) {
            // 何個あるのかわからへんねんけど、試した感じは１個みたいやからとりあえず先頭のを返す    //虽然不知道有几个，但是尝试的感觉就是一个像一个
            const uvc_input_terminal_t *input_terminals = uvc_get_input_terminals(
                    mDeviceHandle);   //UVC得到输入端子
...
///////////////////////////////////////////////////////////////
/**
*下列函数会通过paramget_func_i16来接收传递过来的 指针函数 , 通过调用该指针函数方法 , 来控制相机 ;
*传递过来的函数为libuvc/src/ctrl.c里的函数实体 ,  比如 : uvc_get_scanning_mode等 ;
*/

//          update ctrl values : 更新控制值
/**
* uvc_device_handle_t   *devh :
* control_value_t       &values :
* paramget_func_i16     get_func :   uvc_get_roll_abs , uvc_get_backlight_compensation , uvc_get_brightness , uvc_get_hue
*
*/
static uvc_error_t update_ctrl_values(uvc_device_handle_t *devh, control_value_t &values,
    ...
    if (!values.min && !values.max) {   //判断数据的最大值或最小值是否有值 ;
        int16_t value;
        ret = get_func(devh, &value, UVC_GET_MIN);  //获取最小值
        if (LIKELY(!ret)) {
            values.min = value;
            LOGV("update_params:min value=%d,min=%d", value, values.min);
            ret = get_func(devh, &value, UVC_GET_MAX);  //获取最大值
            if (LIKELY(!ret)) {
                values.max = value;
                LOGV("update_params:max value=%d,max=%d", value, values.max);
                ret = get_func(devh, &value, UVC_GET_DEF);  //获取默认值
                if (LIKELY(!ret)) {
                    values.def = value;
...
/*
* paramget_func_u16 get_func :  uvc_get_iris_abs , uvc_get_zoom_abs , uvc_get_contrast ,
                                uvc_get_gain , uvc_get_saturation , uvc_get_sharpness ,
                                uvc_get_gamma , uvc_get_white_balance_temperature ,
                                uvc_get_digital_multiplier , uvc_get_digital_multiplier_limit
*/
static uvc_error_t update_ctrl_values(uvc_device_handle_t *devh, control_value_t &values,
...
/*
* paramget_func_u8 get_func :    uvc_get_ae_priority , uvc_get_focus_simple_range , uvc_get_focus_auto ,
                                uvc_get_iris_rel , uvc_get_privacy , uvc_get_contrast_auto , uvc_get_powerline_freqency ,
                                uvc_get_hue_auto , uvc_get_white_balance_temperature_auto , uvc_get_white_balance_component_auto ,
                                uvc_get_analog_video_standard , uvc_get_analog_video_lockstate , uvc_get_input_select
*/
static uvc_error_t update_ctrl_values(uvc_device_handle_t *devh, control_value_t &values,
...
/*
* paramget_func_i8u8 get_func :   uvc_get_focus_rel , uvc_get_roll_rel
*/
static uvc_error_t update_ctrl_values(uvc_device_handle_t *devh, control_value_t &values,
...
/*
* paramget_func_i8u8u8 get_func :  uvc_get_zoom_rel
*/
static uvc_error_t update_ctrl_values(uvc_device_handle_t *devh, control_value_t &values,
...
/*
* paramget_func_u32 get_func :  uvc_get_white_balance_component
*/
static uvc_error_t update_ctrl_values(uvc_device_handle_t *devh, control_value_t &values,
...
/*
* paramget_func_i32i32 get_func :  无
*/
static uvc_error_t
...
//////////////////////////////////////////////////////////////////////////
//      internal Set Ctrl Value : 内部设置控制值
...
int UVCCamera::internalSetCtrlValue(control_value_t &values, uint8_t value,
    ...
    if (LIKELY(!ret)) {    // 正常に最小・最大値を取得出来た時         //正常最小值获得的时候
...
int UVCCamera::internalSetCtrlValue(control_value_t &values, uint32_t value,
    ...
    int ret = update_ctrl_values(mDeviceHandle, values, get_func);
    if (LIKELY(!ret)) {    // 正常に最小・最大値を取得出来た時         //正常最小值获得的时候    //即返回值为0 ;
        value = value < values.min
                ? values.min
                : (value > values.max ? values.max : value);
        set_func(mDeviceHandle, value);
...
* int8_t value :
*
* paramget_func_i8 get_func : 无
* paramset_func_i8 set_func : 无
*
*/
...
* uint8_t value2 :
*
* paramget_func_u8u8 get_func : 无
* paramset_func_u8u8 set_func : 无
*
*/
...
// オートフォーカスのon/off状態を取得     //自动聚焦的on / off状态
...
int UVCCamera::setColor(uint32_t color) {
    ENTER();
...
int UVCCamera::setGray(int gray) {
    ENTER();

    int ret = 0;
    if (LIKELY(mPreview)) {
        mPreview->setGray(gray);
    }
    RETURN(ret, int);
}

// コントラストの現在値を取得        //对比度的实际值
...
// オートホワイトバランス(temp)をon/off     //自动白平衡（temp）on / off
...
// オートホワイトバランス(temp)のon/off状態を取得        //自动白平衡（temp）的on / off状态取得
...
// オートホワイトバランス(compo)をon/off        //自动白平衡（compo）on / off
...
// オートホワイトバランス(compo)のon/off状態を取得   //自动白平衡（compo）的on / off状态取得
...
// ホワイトバランス色温度の現在値を取得       //白色平衡色温度的现在值
...
// ホワイトバランスcompoの現在値を取得     //白平衡compo现在值
...
int UVCCamera::setGamma(int gamma) {
    ...
    if (mPUSupports & PU_GAMMA) {
        if (LOCAL_DEBUG) {
            //LOGI("set gamma:%d", gamma);
        }
        int ret = 0;
        if (LIKELY(mPreview)) {
            mPreview->setGray(gamma);
        }
...
// 電源周波数によるチラつき補正を設定        //根据电源表示的chira修正
...
// 電源周波数によるチラつき補正値を取得       //根据电源次数的chira补正值
...
//======================================================================
// digital multiplier調整     //数字multiplier調整
...
// digital multiplierを設定        //数字multiplierを設定
...
// digital multiplierの現在値を取得        //数字multiplier现在值
...
// digital multiplier limitを設定      //limitを設定数字乘法器
...
//======================================================================
// AnalogVideoLoackStatus       //模拟视频锁定状态
