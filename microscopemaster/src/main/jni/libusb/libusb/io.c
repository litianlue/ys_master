 ...
 */
int API_EXPORTED libusb_submit_transfer(struct libusb_transfer *transfer) {     //2016.08.26 : 提交传输。
 ...
 * will attempt to take the lock. */
 //2016.058.30 : usb数据传输完成处理操作
int usbi_handle_transfer_completion(struct usbi_transfer *itransfer,    //usb数据传输完成处理操作
	enum libusb_transfer_status status) {

	struct libusb_transfer *transfer =
		USBI_TRANSFER_TO_LIBUSB_TRANSFER(itransfer);        //定义在libusb.h 将itransfer转换为transfer的宏定义
	struct libusb_context *ctx = TRANSFER_CTX(transfer);
	...
	if (status == LIBUSB_TRANSFER_COMPLETED
		...
		if (rqlen != itransfer->transferred) {	// XXX itransfer->transferred is almost always zero on iso transfer mode...
			usbi_dbg("interpreting short transfer as error");   //以错误解释短转
	...
	transfer->actual_length = itransfer->transferred;	// XXX therefore transfer->actual_length is also almost always zero on iso transfer mode    //XXX因此转移-> actual_length也几乎总是在ISO传输模式零
	usbi_dbg("transfer %p has callback %p", transfer, transfer->callback);

	if LIKELY(transfer->callback)
		transfer->callback(transfer);       //2016.08.30 : _uvc_stream_callback()回调函数在此被回调 ;

	/* transfer might have been freed by the above call, do not use from this point.
	 传输可能已被上述调用释放，不要使用从这一点。  */
