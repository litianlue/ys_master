 ...
 * API_EXPORTED.
     libusb_call必须在两libusb的定义和声明定义
     功能。你会认为，宣言就够了，但是Cygwin会
     抱怨冲突的类型，除非这两个标记的方式。
     这个宏的布局也是很重要的，它必须出现在
     返回类型，在函数名。见内部文件
     api_exported。
...
enum libusb_transfer_status {
	/** Transfer completed without error. Note that this does not indicate that the entire amount of requested data was transferred.
	 无差错传输完成。请注意，这并不表示所请求的数据的全部金额被传输。 */
	...
	/** For bulk/interrupt endpoints: halt condition detected (endpoint stalled). For control endpoints: control request not supported.
	 对于批量/中断端点：停止状态检测（端点停滞）。用于控制端点：不支持的控制请求。  */
	...
	/* NB! Remember to update libusb_error_name() when adding new status codes here.
	 NB！记得更新libusb_error_name()当添加新的状态代码在这里。   */
};
 ...
 * it back to the user.
 ...
 它回到用户。
 */
struct libusb_transfer {            //2016.08.26 : usb的传输数据结构结构体 :libusb_transfer
	/** Handle of the device that this transfer will be submitted to
	此传递的设备的句柄将被提交给*/
	libusb_device_handle *dev_handle;

	/** A bitwise OR combination of \ref libusb_transfer_flags.
	按位或组合\参考libusb_transfer_flags。*/
	uint8_t flags;
	...
	从“裁判libusb_transfer_type端点类型*/
	unsigned char type;

	/** Timeout for this transfer in millseconds. A value of 0 indicates no timeout.
	 这种传递在millseconds超时。值为0表示没有
     超时。*/
	unsigned int timeout;

	/** The status of the transfer. Read-only, and only for use within transfer callback function.
	 * 传输的状态。只读取，并且只用于传递回调函数。
	 *
	 * If this is an isochronous transfer, this field may read COMPLETED even
	 * if there were errors in the frames. Use the \ref libusb_iso_packet_descriptor::status "status" field in each packet to determine if errors occurred.
	 *
	 如果这是一个同步传输，这场可能读完成甚至
     如果框架中有错误。使用的参考libusb_iso_packet_descriptor：：状态“状态”领域中的每个分组以确定错误发生。
	 */
	enum libusb_transfer_status status;
	...
	/** Actual length of data that was transferred. Read-only, and only for use within transfer callback function. Not valid for isochronous endpoint transfers.
	 传输的数据的实际长度。只读取，并且只用于传递回调函数。没有有效的同步端点传输。  */
	int actual_length;

	/** Callback function. This will be invoked when the transfer completes, fails, or is cancelled.
	 回调函数。当传输完成后，将被调用，失败，或被取消。  */
	libusb_transfer_cb_fn callback;

	/** User context data to pass to the callback function.
	用户上下文数据传递给回调函数。*/
	void *user_data;        //user_data强转指向uvc_stream_handle_t类型的数据 ,最终buffer数据缓冲区的数据转存到user_data ;

	/** Data buffer
	数据缓冲区*/
	unsigned char *buffer;      //usb传输的数据都缓冲在 !
 ...
 *  - LIBUSB_LOG_LEVEL_ERROR (1)   : error messages are printed to stderr       //错误消息打印到标准错误
 ...
 * \param timeout timeout for the transfer in milliseconds
 */
static inline void libusb_fill_interrupt_transfer(  //只是将传入的参数赋值给transfer的各成员 ; 重要
	struct libusb_transfer *transfer, libusb_device_handle *dev_handle,
...
static inline void libusb_fill_iso_transfer(    //只是将传入的参数赋值给transfer的各成员 ; 重要 ;
    struct libusb_transfer *transfer,
	libusb_device_handle *dev_handle, unsigned char endpoint,
