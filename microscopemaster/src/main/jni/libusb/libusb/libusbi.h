 ...
 *   return_type LIBUSB_CALL function_name(params);
     在所有的公共功能标记libusb的代码，如下：
     return_type api_exported function_name（参数）{…}
     但如果函数返回一个指针，则标记如下：
     default_visibility return_type * libusb_call function_name（参数）{…}
     在libusb公共头，标记所有声明为：
     return_type libusb_call function_name（参数）；
 */
...
static inline void list_del(struct list_head *entry) {
	if (!list_empty(entry)) {	// XXX add Mr_Wen@yeespec because sometimes crash
...
#define USBI_TRANSFER_TO_LIBUSB_TRANSFER(transfer) /*将itransfer转换为transfer的宏定义*/\
