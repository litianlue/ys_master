...
static void dump_urb(int ix, int fd, struct usbfs_urb *urb) {
	...
	LOGI("O_ACCMODE:%x", ret & O_ACCMODE);				// 0:読み込み専用, 1:書き込み専用, 2;読み書き可      //0：读取专用，1：写入专用，2 :读写均可
	LOGI("ノンブロッキングかどうか:%d", ret & O_NONBLOCK);	// 0:ブロッキング        //0，结块
...
static struct linux_device_priv *_device_priv(struct libusb_device *device);
...
static int linux_get_parent_info(struct libusb_device *dev,
...
static int handle_bulk_completion(struct libusb_device_handle *handle,	    //处理批量传输完成事件 // XXX added Mr_Wen
	...
	case -EPIPE:
		...
		op_clear_halt(handle, urb->endpoint);	// XXX added Mr_Wen
		goto cancel_remaining;
	case -EOVERFLOW:
...
completed:
	...
	return CANCELLED == tpriv->reap_action ?
		...
		usbi_handle_transfer_completion(itransfer, tpriv->reap_status);     //处理批量传输完成事件
}

static int handle_iso_completion(struct libusb_device_handle *handle,	//处理同步传输ISO完成事件 // XXX added Mr_Wen
		struct usbi_transfer *itransfer,
		struct usbfs_urb *urb) {
	struct libusb_transfer *transfer = USBI_TRANSFER_TO_LIBUSB_TRANSFER(itransfer);
	struct linux_transfer_priv *tpriv = usbi_transfer_get_os_priv(itransfer);
	...
	if (UNLIKELY(tpriv->reap_action != NORMAL)) { /* cancelled or submit_fail */
		...
		if (tpriv->num_retired == num_urbs) {
			...
			if (tpriv->reap_action == CANCELLED) {
				...
				usbi_mutex_unlock(&itransfer->lock);
				return usbi_handle_transfer_completion(itransfer, LIBUSB_TRANSFER_ERROR);   //处理同步传输ISO完成事件
			}
		}
		goto out;
	}
	...
	if (urb_idx == num_urbs) {
		...
		free_iso_urbs(tpriv);
		usbi_mutex_unlock(&itransfer->lock);
		return usbi_handle_transfer_completion(itransfer, status);       //处理同步传输ISO完成事件
	}
...
static int handle_control_completion(struct libusb_device_handle *handle,	// 处理控制完成事件 // XXX added Mr_Wen   //XXX加入saki
		struct usbi_transfer *itransfer,
		struct usbfs_urb *urb) {
	struct linux_transfer_priv *tpriv = usbi_transfer_get_os_priv(itransfer);
	int status;
	...
	usbi_mutex_unlock(&itransfer->lock);
	return usbi_handle_transfer_completion(itransfer, status);      //处理控制完成事件
}

static int reap_for_handle(struct libusb_device_handle *handle) {       //2016.08.30 : 获得处理 :
	struct linux_device_handle_priv *hpriv = _device_handle_priv(handle);
	int r;
	struct usbfs_urb *urb;
	struct usbi_transfer *itransfer;
    ...
    参数个数如下：int ioctl(int fd, int cmd, …)；
    fd :就是用户程序打开设备时使用open函数返回的文件标示符，
    cmd :就是用户程序对设备的控制命令，
    可变参数 : 后面的省略号，那是一些补充参数，一般最多一个，有或没有是和cmd的意义相关的。

    ioctl函数是文件结构中的一个属性分量，就是说如果你的驱动程序提供了对ioctl的支持，
    用户就能在用户程序中使用ioctl函数控制设备的I/O通道。

    用户程式所作的只是通过命令码告诉驱动程式他想做什么，
    至于怎么解释这些命令和怎么实现这些命令，这都是驱动程式要做的事情。
    */
	r = ioctl(hpriv->fd, IOCTL_USBFS_REAPURBNDELAY, &urb);  //IOCTL_USBFS_REAPURBNDELAY宏定义(linux的命令) 用于向驱动程序写入数据命令 ;

	if (r == -1 && errno == EAGAIN)     //返回值：成功为0，出错为-1
		return 1;
	if (UNLIKELY(r < 0)) {
		if (errno == ENODEV)
			return LIBUSB_ERROR_NO_DEVICE;
	...
	transfer = USBI_TRANSFER_TO_LIBUSB_TRANSFER(itransfer); //定义在libusb.h 将itransfer转换为transfer的宏定义

	usbi_dbg("urb type=%d status=%d transferred=%d",
		urb->type, urb->status, urb->actual_length);

	switch (transfer->type) {
	case LIBUSB_TRANSFER_TYPE_ISOCHRONOUS:  //libusb同步传输类型
		return handle_iso_completion(handle, itransfer, urb);       //处理同步传输ISO完成事件
	case LIBUSB_TRANSFER_TYPE_BULK:     //libusb数据块传输类型
	case LIBUSB_TRANSFER_TYPE_BULK_STREAM:  //libusb数据流块传输类型
	case LIBUSB_TRANSFER_TYPE_INTERRUPT:    //libusb中断传输类型
		return handle_bulk_completion(handle, itransfer, urb);      //处理批量传输完成事件
	case LIBUSB_TRANSFER_TYPE_CONTROL:
		return handle_control_completion(handle, itransfer, urb);   //处理控制完成事件
	default:
		usbi_err(HANDLE_CTX(handle),
			"unrecognised endpoint type %x", transfer->type);   //无法识别的端点类型
		return LIBUSB_ERROR_OTHER;
	}
}
...
static int op_handle_events(struct libusb_context *ctx, struct pollfd *fds,
	...
	for (i = 0; i < nfds && num_ready > 0; i++) {
		...
		do {
			r = reap_for_handle(handle);    //调用处理事件的函数 ;
		} while (r == 0);
...
const struct usbi_os_backend android_usbfs_backend = {
	...
	.handle_events = op_handle_events,      //事件处理函数
