...
struct usbfs_urb {
	...
	int start_frame;
	union {
		int number_of_packets;	/* Only used for isoc urbs */       //仅用于协会城市
		unsigned int stream_id;	/* Only used with bulk streams */   //只用于批量流
	};
	int error_count;
	unsigned int signr;
	void *usercontext;
...
struct usbfs_streams {
	unsigned int num_streams; /* Not used by USBDEVFS_FREE_STREAMS */   //通过usbdevfs_free_streams不习惯
	unsigned int num_eps;
	unsigned char eps[0];
};

/*
_IOR 构造从驱动程序中读取数据的命令编号
_IOW 用于向驱动程序写入数据命令
_IOWR 用于双向传输
*/
